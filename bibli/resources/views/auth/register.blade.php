        ...
        <div style="width: 100%; max-width: 400px; background: white; padding: 2rem; border-radius: 0.75rem; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <!-- User Role -->
                    <label style="display: flex; align-items: center; margin-bottom: 0.5rem; cursor: pointer; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.5rem; background-color: #f9fafb;">
                        <input type="radio" name="role" value="user" {{ old('role', 'user') == 'user' ? 'checked' : '' }}
                               style="margin-right: 0.75rem; color: #6366f1;">
                        <div>
                            <div style="font-weight: 500; color: #111827; font-size: 0.875rem;">👤 Usuario</div>
                            <div style="color: #6b7280; font-size: 0.75rem;">Comprar y leer libros digitales</div>
                        </div>
                    </label>

                    <!-- Admin Role -->
                    <label style="display: flex; align-items: center; cursor: pointer; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.5rem; background-color: #f9fafb;">
                        <input type="radio" name="role" value="admin" {{ old('role') == 'admin' ? 'checked' : '' }}
                               style="margin-right: 0.75rem; color: #6366f1;">
                        <div>
                            <div style="font-weight: 500; color: #111827; font-size: 0.875rem;">⚙️ Administrador</div>
                            <div style="color: #6b7280; font-size: 0.75rem;">Gestionar libros, usuarios y ventas</div>
                        </div>
                    </label>
